...
import type { PaymentParams } from '../../../../models/Models';
import { Button } from '@mui/material';
import { OrderProducts } from '../OrderProducts/OrderProducts';
import { useOrderStore } from '../../store/OrderStore';
import { NoProductsToOrder } from '../noProducts/NoProducts';
import { generatePayment } from '../../api/robokassaApi';
import type { PayRequest, ReceiptItem } from '../../api/robokassaApi';
import { useGetProductsPrice } from '../../store/OrderComputedValues';
import { useGetProductsQuantity } from '../../store/OrderComputedValues';
			...
			{orderProducts.length > 0 ? (
				<>
					...
					<Paper square className={styles.orderSummary}>
						...
						<p>
							Получатель:{' '}
							{`${clientInfo.lastName} ${clientInfo.firstName} ${clientInfo.fatherName}`}
						</p>
						...
						<div>
							<div className={styles.row}>
								{shippingType === 'cdek' && <div>Доставка (СДЭК)</div>}
								{shippingType === 'yandex' && (
									<div>Яндекс доставка оплачивается отдельно</div>
								)}
								{shippingType === 'pickup' && <div>Самовывоз</div>}
								{shippingType === 'cdek' && <div>{deliveryPrice} ₽</div>}
								{shippingType === 'pickup' && <div>{deliveryPrice} ₽</div>}
							</div>
							{shippingType !== 'yandex' && (
								<div className={styles.row}>
									<div>Итого</div>
							...
							{shippingType === 'pickup' && (
								<p>
									Самовывоз осуществляется из магазина "Твоя полка" в тц
									"Пирамида" по адресу: ул.Кирова д.19а (ежедневно с 10:00 до
									20:00)
								</p>
							)}
						</div>
						<p>При наличии вопросов обращайтесь по электронной почте:</p>
						<a href={`mailto:${process.env.NEXT_PUBLIC_SHOP_EMAIL}`}>
